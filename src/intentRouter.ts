...
export async function routeQuestionWithGemini(chatId: number, text: string): Promise<RouterDecision> {
  ...
  const prompt = [
    ...
    '- search: buscar algo guardado por palabra clave, cuando no es claramente sueldo ni finanzas. Args: query (2-4 palabras clave del tema, SIN palabras de relleno como "que", "sabés", "sobre", "mail" — ej para "qué sabés sobre el mail del control de cambios" mandá query="control de cambios", no la pregunta completa).',
